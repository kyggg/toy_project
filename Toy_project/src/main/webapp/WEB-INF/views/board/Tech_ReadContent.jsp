            ...
            <div class="col">
                    ...
                    <table class="table table-bordered" style="height: auto; align-self: center; margin: 0px;">
                        <div class="col">
                            <tr style="text-align: center;">
                                ...
                                <td style="width:15%;">${content.memberVo.member_name }</td>

                                <td style="width:15%; background: rgb(202, 200, 200);">조회수</td>
                                <td>${content.techVo.tech_readcount }</td>
                        ...
                        <div class="col">
                            <tr style="text-align: center;">
                                ...
                                <td colspan="5">${content.techVo.tech_title }</td>
                            </tr>
                        </div>
                        <div class="col col-md-offset-2">
                            <tr style="height: 320px;">
                                <td colspan="6">
   								<c:forEach items="${content.techImageVoList }" var="techImageVo">
									<img src="/uploadFolder/${techImageVo.image_url }"><br>
								</c:forEach><br>
                                   ${content.techVo.tech_content }   
                                </td>
                            </tr>
                        </div>
                    </table>
                ...
                <div class="row mt-3">
		                ...
		                <c:if test="${!empty sessionUser }">
		                    <c:choose>
						    	<c:when test="${!empty sessionUser && sessionUser.member_no == content.like.member_no  }">
						    		<a href="deleteLike.do?member_no=${sessionUser.member_no }&&tech_no=${content.techVo.tech_no}">
		                        <button type="button" class="btn btn-outline-danger">좋아요 <i class="bi bi-heart-fill"></i></button></a>
						    	</c:when>
						    	<c:otherwise>
						    		<a href="setLike.do?member_no=${sessionUser.member_no }&&tech_no=${content.techVo.tech_no}">
						    		<button type="button" class="btn btn-outline-danger">좋아요 <i class="bi bi-heart"></i></button></a>
						    	</c:otherwise>
						    </c:choose>
						 </c:if>
                    </div>
                    <div class="col" style="text-align:end;">
                        <c:if test="${!empty sessionUser && sessionUser.member_no == content.memberVo.member_no }">
                       	<a href="./deleteContentProcess.do?tech_no=${content.techVo.tech_no }"><button type="button" class="btn btn-danger" style="border-radius:5px;">삭제</button></a>
                        <a href="./Tech_Update.do?tech_no=${content.techVo.tech_no }"><button type="button" class="btn btn-success" style="border-radius:5px; margin-left: 10px;">수정</button></a>
                        </c:if>
                        <a href="./Tech_Html.do"><button type="button" class="btn btn-success" style="border-radius:5px; margin-left: 10px;">목록</button></a>
                    </div>
                </div>
                ...
                <div class="row mt-3">
					...
					<form action="writeCommentProcess.do" method="post">										
                          ...
                          <c:if test="${!empty sessionUser }">
                               <input type="text" name="Tcomment_content" class="form-control" placeholder="댓글내용" aria-describedby="button-addon2">
							<input type="hidden" name="tech_no" value="${content.techVo.tech_no}">
							<input type="hidden" name="member_no" value="${sessionUser.member_no}">								
							<input type = "submit" class="btn btn-outline-secondary" value="댓글등록" id="button-addon2">	                                
						</c:if>
                         </div>
      				</form>
                    </div>
                ...
                <div class="row">
                    <div class="col">
                        ...
                        <tr style="text-align: center;">
                            <td style="width:15%; background: rgb(202, 200, 200);">${com.memberVo.member_name }</td>
                            <td colspan="9" style="text-align: start;">${com.techCommentVo.tcomment_content }</td>
                            <td style="width:10%;"><fmt:formatDate value="${com.techCommentVo.tcomment_date }" pattern="MM-dd"/></td>
                        </tr>
	                    </c:forEach>
		  				</c:if>
                    </table>
